...
```
aihc job create --name "ui-test-no-alert-success-rerun2" --framework pytorch --image "registry.baidubce.com/aihcp-public/pytorch:22.08-py3" --enable-rdma=true --enable-bccl=false --enable-fault-tolerance=true --fault-tolerance-args="--enable-replace=false --enable-hang-detection=false --hang-detection-log-timeout-minutes=0 --hang-detection-startup-toleration-minutes=0 --hang-detection-stack-timeout-minutes=0 --max-num-of-unconditional-retry=0 --unconditional-retry-observe-seconds=0 --enable-use-nodes-of-last-job=false --enable-checkpoint-migration=false" --priority normal --replicas 1 --env "AIHC_JOB_NAME=ui-test-no-alert-success-rerun2" --env "AIHC_TENSORBOARD_LOG_PATH=/mnt/ui-test-no-alert-success-rerun2/output/training_logs" --env "NCCL_IB_DISABLE=1" --ds-type empty --ds-mountpath "/dev/shm" --ds-name "devshm" --ds-size-limit 10 --ds-type pfs --ds-mountpath "/mnt" --ds-name "pfs-7xWeAt" --ds-size-limit 1000 --ds-pfs-cluster-ip 192.168.12.142 --ds-pfs-cluster-port 8888 --command "sleep 5
exit 0" --label "aijob.cce.baidubce.com/create-from-aihcp=true" --enable-tensorboard=true --tensorboard-log-path "/mnt/ui-test-no-alert-success-rerun2/output/training_logs"
```