...
export function formatRequestParams(params: any): any {
  ...
  if (params.jobSpec) {
    ...
    const envs = params.jobSpec.Master?.env ? Object.entries(params.jobSpec.Master.env).filter(([name, _value]) => !['AIHC_JOB_NAME', 'AIHC_TENSORBOARD_LOG_PATH', 'MASTER', 'NCCL_DEBUG', 'NCCL_IB_DISABLE'].includes(name)).map(([name, value]) => ({
      name,
      value: String(value)
    })) : []
    const jobSpec: any = {
  ...
  // 处理 labels
  if (params.labels) {
    const labels = Object.entries(params.labels).filter(([key, _value]) => key !== 'aijob.cce.baidubce.com/create-from-aihcp').map(([key, value]) => ({
      key,
      value: String(value)
    }));
